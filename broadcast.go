...
func broadcastReceive(iface *net.Interface, sendPort int, sendMsg []byte, handler func(n int, addr *net.UDPAddr, buf []byte)) error {
	...
	buf := make([]byte, 1024)
	rconn.SetDeadline(time.Now().Add(3 * time.Second))
	for {
